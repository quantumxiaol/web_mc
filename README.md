...
- `W/A/S/D` 移动
- `G` 切换飞行 / 步行
- 35 种可放置方块，包含地形、建筑、自然、矿石、水、岩浆和黑曜石
...
- 图形档位、快捷栏配置和手动保存的世界状态会写入浏览器 `localStorage`
- v2 世界存档只保存 edited chunks，并记录玩家位置、日夜时间、流体调试状态和图形档位
...
- 水和岩浆接触会让源岩浆转成黑曜石、流动岩浆转成石头；岩浆会吞掉树叶、蘑菇和仙人掌，雪会融掉，冰会融成水
- 玩家进入水中会减速并可上浮，进入岩浆会进一步减速并显示橙红 overlay；F3 会显示当前液体环境
...
- `Vitest` 覆盖 DDA raycast、流体反应、局部发光源、玩家液体状态、edited chunk 导入/导出和日夜循环基础规则
